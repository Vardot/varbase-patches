# Changelog — Vardot/varbase-patches (`9.2.x`)

All notable changes on the `9.2.x` branch of [`Vardot/varbase-patches`](https://github.com/Vardot/varbase-patches), newest first.
...
- Restore the php-css-lint library patch removed in [#648](https://github.com/Vardot/varbase-patches/pull/648) and released in 9.2.101 - it was removed in error: this branch also serves Varbase 9.1 sites (`vardot/varbase` 9.1.x requires `vardot/varbase-patches:~9.2.0`), which install php-css-lint **v3.2.0**, whose `\CssLint\Properties $oProperties = null` constructor is exactly what the patch fixes ([#653](https://github.com/Vardot/varbase-patches/issues/653))
...
- Remove a patch for the CTools (Chaos Tool Suite) module on fix: #3492432 PHP 8.4 nullable types in MaskContentEntityStorage::doLoadMultiple() (fixed upstream, released in ctools 4.1.1) (#555)
...
- Remove the Social Auth patches that do not apply on the 9.2.x line (social_auth ~4.1) (#400)
- Add missing patches for shared modules on Drupal ~11.3 / PHP 8.4 (ported from 10.1.x) (#399)
- fix: patch layout_library isSupported for D11 (#3561874, port from 10.1.x) (#398)
- fix: patch jsonapi_extras for install error #3508142 (port from 10.1.x)
- fix: remove non-applying social_auth PHP 8.4 patch (#3564906) — fix is in social_auth 4.1.2

## [9.2.92] - 2026-06-28

- docs: PR/MR template with Checkpoints (no UX/UI line)
- fix: [9.2.x] apply vardot/drupal-core-patches by default (allowed-dependency-patches) (#393)
- docs: add PR/MR template with Checkpoints
- task: require vardot/drupal-core-patches (~10 || ~11 || ~12); move drupal/core patches out (#386)
...
- Issue #366: Backport in-repo AI agent + skills (AGENTS.md / CLAUDE.md / .claude/) to 9.2.x
- Issue #365: Rename docs/index.md to docs/README.md and refresh docs landing for branch 9.2.x
...
- Remove a patch for the Page Manager module on fix: #3536704 PHP 8.4 implicit nullable parameters #343
...
- Add a patch for the Panels module on fix: #3534531 php 8.4 compatibility issue #259
- Add patch for the Page Manager module on fix: #3536704 PHP 8.4 implicit nullable parameters #258
...
- Restrict old list of Drupal core's patches to Drupal ~10.2.0 in the 9.2.x branch and release the 9.2.12 tag #41
...
- Remove all patches for drupal/ckeditor in the 9.2.x branch #20
...
- Initial tracked release on the `9.2.x` branch.
